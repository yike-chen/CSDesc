...
def get_whole_training_set(onlyDB=False, matfile='oxRobocar_train_day.mat'):
    structFile = join(struct_dir, matfile)
    return WholeDatasetFromStruct(structFile,
                             input_transform=input_transform(),
                             onlyDB=onlyDB)

def get_whole_val_set(matfile='oxRobocar_val_day.mat'):
    structFile = join(struct_dir, matfile)
    return WholeDatasetFromStruct(structFile,
...
def get_training_query_set(margin=0.1, matfile='oxRobocar_train_day.mat'):
    structFile = join(struct_dir, matfile)
    return QueryDatasetFromStruct(structFile,
                             input_transform=input_transform(), margin=margin)

def get_val_query_set():
    structFile = join(struct_dir, 'oxRobocar_val.mat')
    return QueryDatasetFromStruct(structFile,
                             input_transform=input_transform())
...
def parse_dbStruct(path):
    ...
    if 'single' in path:
        dataset = 'oxfordcarday'
    else:
        dataset = 'oxfordcarnight'
